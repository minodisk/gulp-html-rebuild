var PLUGIN_NAME, Parser, PassThrough, PluginError, createAttrStr, replaceExtension, through, voidElements, _ref;
...
module.exports = function(opts) {
  ...
  return through.obj(function(file, enc, callback) {
    ...
    if (file.isStream()) {
      ...
      parser = new Parser({
        onprocessinginstruction: function(name, value) {
          return stream.write(opts.onprocessinginstruction(name, value));
        },
        onopentag: function(name, attrs) {
          return stream.write(opts.onopentag(name, attrs, createAttrStr));
        },
        ontext: function(text) {
          return stream.write(opts.ontext(text));
        },
        onclosetag: function(name, attrs) {
          if (!parser._options.xmlMode && name in voidElements) {
            return;
          }
          return stream.write(opts.onclosetag(name, attrs, createAttrStr));
        },
        oncomment: function(value) {
          return stream.write(opts.oncomment(value));
        },
        onend: function() {
          return stream.end();
        }
